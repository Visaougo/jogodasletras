...
export default function App() {
  ...
  const handleUnlockAchievement = (ach: Achievement) => {
      setUser(prev => {
          ...
          let newEquippedSkin = prev.equippedWolfSkin;

          if (ach.reward) {
              newEmeralds += ach.reward.emeralds;
              if (ach.reward.item && !newInventory.includes(ach.reward.item)) {
                  newInventory.push(ach.reward.item);
                  // Optional: Auto equip if it's a skin? No, just unlock.
                  if (ach.reward.item.startsWith('skin_wolf_')) {
                      // Maybe auto-equip for cool factor? Let's leave it to the user.
                  }
              }
          }
  ...
  const handleStartGame = (module: BiomeModule) => {
    ...
    // Using LETTERS as a baseline check instead of undefined WORLD_CORE
    if (isLocked && module.type !== ModuleType.LETTERS && !isMiniGame) {
        playSFX('error');
        return;
    }
  ...
  const handleGameComplete = (xpEarned: number, emeraldsEarned: number, learnedWords: string[], droppedCard?: string, newAdaptiveStats?: any, duration?: number) => {
    setUser(prev => {
        ...
        if (activeModule?.type === ModuleType.SYLLABLES || activeModule?.type === ModuleType.MINE) {
            newGameStats.syllableGamesPlayed = (newGameStats.syllableGamesPlayed || 0) + 1;
        }
        if (duration) {
            if (newGameStats.fastestCompletionTime === 0 || duration < newGameStats.fastestCompletionTime) {
                newGameStats.fastestCompletionTime = duration;
            }
        }
  ...
  const droppedCardData = COLLECTIBLE_CARDS.find(c => c.id === droppedCardId);
         ...
         <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[60] p-4" onClick={() => setDroppedCardId(null)}>
             <div className="flex flex-col items-center animate-pop-in">
                 ...
                 <div className="transform scale-125 mb-8">
                     <CardComponent card={droppedCardData} isNew={true} />
                 </div>
