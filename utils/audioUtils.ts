...
export const playSFX = (type: 'click' | 'success' | 'error' | 'pop' | 'wolf_howl' | 'confirm' | 'chest_open' | 'portal' | 'step' | 'secret' | 'wake' | 'win') => {
  try {
    ...
    switch (type) {
      ...
      case 'win':
        ...
        gainNode.gain.setValueAtTime(0.1 * volumeMultiplier, now);
        gainNode.gain.linearRampToValueAtTime(0.1 * volumeMultiplier, now + 0.6);
